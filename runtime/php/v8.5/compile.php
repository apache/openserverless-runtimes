...
function sources(string $src)
{
    ...
    if (file_exists($src . '/composer.json')) {
        // Run composer if exists locally composer.json with optimized autoloader
        exec("export HOME=$src; cd $src; composer install --no-progress --no-dev -o --classmap-authoritative -q");
    }

    // put vendor in the right place if it doesn't exist
    if (!is_dir($src . '/vendor')) {
        exec('cp -a /app/.composer/vendor ' . escapeshellarg($src . '/vendor'));
    }
}
